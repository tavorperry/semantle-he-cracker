...
const increaseCounters = () => { //TODO: export to another module (ip_runner (range))
  for (let i=0; i < counters.length; ++i) {
   if (counters[i] < 2560) {
    ++counters[i];
    return;
   }
   if (counters[counters.length-1] >= 2560) {
...
const makeGetRequest = (path, word) => {
  increaseCounters();
  const ip = `${counters[0]}'.'${counters[1]}'.'${counters[2]}'.'${counters[3]}'.'${counters[4]}`;
